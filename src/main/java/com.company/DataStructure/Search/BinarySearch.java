...
public class BinarySearch {
    ...
    public static int binarySearch(int[] arr, int targetValue, int left, int right) {
        /**
         * 二分法只适用于 有序数组！！！
         * @param arr 有序数组
        ...
        int midIndex = (right + left) / 2; // 中间索引位置
        int midValue = arr[midIndex]; // 中间值

        if (midValue < targetValue) {
            return binarySearch(arr, targetValue, midIndex + 1, right); // 向上递归
        } else if (midValue > targetValue) {
            return binarySearch(arr, targetValue, left, midIndex - 1); // 向下递归
        } else {
            return midIndex; // 相等则找到
        }
    }
